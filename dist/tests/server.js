...
router.post("/redirect", (request, response) => {
    var _a;
    const _b = request.body, { path, sleep } = _b, query = __rest(_b, ["path", "sleep"]);
    const { pathname, query: searchParams } = url__default["default"].parse((_a = path !== null && path !== void 0 ? path : request.query.path) !== null && _a !== void 0 ? _a : "/src/tests/fixtures/one.html", true);
    const enctype = request.get("Content-Type");
    if (enctype) {
        query.enctype = enctype;
    }
    setTimeout(() => response.redirect(303, url__default["default"].format({ pathname, query: Object.assign(Object.assign({}, query), searchParams) })), parseInt(sleep || "0", 10));
});
